...
void CLDecoy::Initialize(void)
{
	m_tInfo.vPos = CObjMgr::Get_Inst()->Get_Last(OBJ_PLAYER)->Get_Info().vPos;
	m_tInfo.vSize = CObjMgr::Get_Inst()->Get_Last(OBJ_PLAYER)->Get_Info().vSize;
	m_tInfo.vDir = { 0, -1 , 0 };
...
void CLDecoy::Late_Update(void)
{
	if (m_bDead)
		m_eRendType = REND_END;
}

void CLDecoy::Render(HDC hDC)
{
...
void CLDecoy::Release(void)
{
}

void CLDecoy::Collide(OBJ_TYPE p_Type, CObj * p_Targ)
{
}
